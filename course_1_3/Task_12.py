...
class SimpleGraph:
    ...
    def WeakVertices(self):
        ...
        for i, node in enumerate(self.vertex):
            if not node:
                continue
            if self.__is_vertice_weak(i):
                weak_vertices.append(node)
        return weak_vertices

    def __is_vertice_weak(self, v: int) -> bool:
        adjacency = []
        for j, elem in enumerate(self.m_adjacency[v]):
            if j == v:
                continue
            if elem == 1:
                adjacency.append(j)
        adjacency_combinations = combinations(adjacency, 2)
        for row, column in adjacency_combinations:

...
class SimpleTree:
    ...
    def EvenTrees(self):
        if self.Root is None:
            return []
        return self.__even_trees(self.Root)

    def __even_trees(self, node: SimpleTreeNode) -> list:
        answer = []
        for i in node.Children:
            answer += self.__even_trees(i)
        subtree_node_count = len(self.__get_nested_nodes(node))
        if subtree_node_count % 2 == 0 and node is not self.Root:

import unittest
from course_1_4__object_oriented_analysis_and_design.Task_2__design_ADT_for_linked_list import LinkedList


class TestHeadOperation(unittest.TestCase):
    pass


class TestTailOperation(unittest.TestCase):
    pass


class TestRightOperation(unittest.TestCase):
    pass


class TestGetOperation(unittest.TestCase):
    pass


class TestPutRightOperation(unittest.TestCase):
    pass


class TestPutLeftOperation(unittest.TestCase):
    pass


class TestRemoveOperation(unittest.TestCase):
    pass


class TestClearOperation(unittest.TestCase):
    pass


class TestSizeOperation(unittest.TestCase):
    pass


class TestAddToEmptyOperation(unittest.TestCase):
    pass
